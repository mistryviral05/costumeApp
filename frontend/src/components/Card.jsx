...
const Card = ({ cupBoard, id, delteCupboard, setCupboards }) => {
    ...
    const handleRename = async () => {
        try {
            const response = await fetch('http://localhost:3000/cupboards/updateCupboard', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        ...
        <tbody  className='shadow-lg border border-collapse w-full    bg-white'>
            <tr className="hover:bg-gray-50 transition-all duration-200">
                ...
                <td className="px-4 py-4 text-center">{cupBoard.space || 'N/A'}</td>
                <td className="px-4 py-4 text-center">{cupBoard.place || 'N/A'}</td>
